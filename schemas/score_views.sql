...
WITH demo_pop AS (
    ...
    WHERE demographics.variable = 'Total population'::text
),
     demo_65p AS (
         SELECT demographics.location,
                demographics.value AS frac65p
         FROM api.demographics
         WHERE demographics.variable = 'Fraction of population over 65'::text
     ),
     daily_case_avg AS (
         SELECT usafacts_covid.fips,
                (max(usafacts_covid.value) - min(usafacts_covid.value))::real / 30::double precision AS recent_cases
         FROM data.usafacts_covid
         WHERE usafacts_covid.dt > (now() - '30 days'::interval)
           AND usafacts_covid.variable_id = (SELECT id from meta.covid_variables where name = 'cases_total')
         GROUP BY usafacts_covid.fips
     ),
     states AS (
         SELECT us_fips.id,
                us_fips.fips,
                ...
                us_fips.county
         FROM meta.us_fips
         WHERE us_fips.fips < 100
     ),
     scores AS (
         SELECT demo_pop.location,
                clamp(demo_pop.population, 0.0::real, 500.0::real) + 10::real * demo_65p.frac65p +
                daily_case_avg.recent_cases::real AS score
         FROM demo_pop
...
SELECT s.location,
       CASE
           WHEN usf.name::text = st.name::text THEN usf.name::text
           ELSE (usf.name::text || ', '::text) || st.name::text
           END AS name,
       s.score
FROM scores s
